devread <- function(filename,rows){
  ...
  d<-read.table(filename,header = T, stringsAsFactors = F,sep=',',nrows=rows)
  d<-tbl_df(d)
...
d$device.capacity.in.GB <- d$device.capacity/(2^20) * d$device.block.size 
