...
feboxplot <- function(df) {
  ...
  from <- min(t2$ts)
  to <- max(t2$ts)
  # remove the directors which have no values.
  dlist <- getdirs(df)
  t2 <- t2[ t2$dir %in% dlist, ]  # This deletes the data
  t2$dir <- factor(t2$dir)    # And this recreates the factors.
  
  # arbitrarily pick 128 colors that aren't white or gray
  colorlist <- grep('white.*|gr[a,e]y.*|ivory.*|snow.*',colors(),invert=TRUE,ignore.case=TRUE,value=TRUE)

  plotcol <- sample(colors(),128)
  boxplot(read~dir,data=t2,las=2,cex.axis=.5,
          outline=FALSE,
          horizontal=TRUE,
          ylab='Director FE',
          col=plotcol,
          main=paste(serialnum,'Read Service Time by Front End Director (mSec)'),
          xlab=paste('start time: ',from,'end time: ',to))

  boxplot(write~dir,data=t2,las=2,cex.axis=.5,
          horizontal=TRUE,
          outline=FALSE,
          ylab='Director FE',
          col = plotcol,
          main=paste(serialnum,'Write Service Time by Front End Director (mSec)'),
          xlab=paste('start time: ',from,'end time: ',to))
}
...
datad <- "/data/0621-stp/output"
serialnum <- "HK195700621"
...
st<-fe %>% 
  ...
  summarize(
    ...
    mean_rrt= mean(average.read.time.ms),
    min_rrt= min(average.read.time.ms),
    max_rrt= max(average.read.time.ms),
    sd_rrt= sd(average.read.time.ms)
  ) %>%
...
fr <- ff %>% select(director.number,average.read.time.ms) %>% 
  filter(!is.na(average.read.time.ms), average.read.time.ms < Inf)
fw <- ff %>% select(director.number,average.write.time.ms) %>% 
  filter(!is.na(average.write.time.ms),average.write.time.ms < Inf )

par(mfrow=c(4,4))
for (dir in dirs) {
  d <- filter(fr, director.number == dir )
  # Reads
  read_time <- d$average.read.time.ms

  # Find everything about the 99th quantile and set = 99th quantile
  # The histogram will be cut at this percentile and the outliers represented as the final bar.
  lastbar <- as.numeric(quantile(read_time,c(0.98)))
  read_time[read_time > lastbar] <- lastbar 
  breaks = 512
  hist(read_time,col='darkblue',breaks,main=paste(serialnum,dir,"Read Svc (mSec)") )
}
stop()
{
  # Writes
